...
public class MagicSquare {
	...
	private boolean[][] squareEmpty;         // keeps track of empty squares
	private boolean[] addsUpCorrectly;  // keeps track of if the row adds up correctly
	private int[][] numberOnSquare;  // The number on a square - should rename it to numberOnSquare eventually
	...
	public void placeQueen(int row, int col) {
		numberOnSquare[row][col] = (minNum + (int)((maxNumber-minNum+1) * Math.random()));  //Instead of true, this should be a number between 1 and the maxNumber possible
	...
	public boolean isSafe(int row, int col) {
		...
		//if row<this.order  and if row==this.order test!
	}
