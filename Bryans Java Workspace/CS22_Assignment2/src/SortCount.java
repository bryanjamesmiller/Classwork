...
public class SortCount {
	...
	{
		//must use compare(arr[0], arr[1]; to compare arr[0] < arr[1]
		//must use move(arr, i, j); instead of arr[i] = arr[j], which is the
		//same as swap (see below) except that it just moves one value to the other index, 
		//not BOTH to each other's index.  It is also more efficient.
		//must use swap(arr, i, j); instead of some other way of swapping
		...
		//the below is bubble sort in a while loop
		...
		while (incr >= 1) 
		{
			...
			//the below is insertion sort also in the while loop.  i need apply the incr part
			//below it the above, and then totally delete the below.
