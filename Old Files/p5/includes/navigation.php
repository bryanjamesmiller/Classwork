<nav id="navigation">
    <ul id="category">
        <li>Website Links
            <ul><li id="nav-1"><a href="http://morpheus.dce.harvard.edu/~bmiller/obscure_4k5z0z3/cscie12_project/index.php">Home</a></li>
                <li id="nav-2"><a href="	http://morpheus.dce.harvard.edu/~bmiller/obscure_4k5z0z3/cscie12_project/spurs.php">San Antonio Spurs</a></li>
                <li id="nav-3"><a href="	http://morpheus.dce.harvard.edu/~bmiller/obscure_4k5z0z3/cscie12_project/rockets.php">Houston Rockets</a></li>
                <li><a href="#">Portland Trailblazers</a></li>
                <li><a href="#">LA Clippers</a></li>
                <li><a href="#">Golden State Warriors</a></li>
                <li><a href="#">Dallas Mavericks</a></li>
                <li><a href="#">Washington Wizards</a></li>
                <li><a href="#">Miami Heat</a></li>
                <li><a href="#">Golden State Warriors</a></li>
                <li><a href="#">Indiana Pacers</a></li>
                <li><a href="#">Brooklyn Nets</a></li>
                <li><a href="#">Toronto Raptors</a></li>
                <li><a href="#">Chicago Bulls</a></li>
                <li><a href="#">Charlotte Bobcats</a></li>
                <li><a href="#">Atlanta Hawks</a></li>
                <li><a href="#">Memphis Grizzlies</a></li>
            </ul>
        </li>
    </ul>
</nav>